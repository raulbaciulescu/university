...
class Evaluation:
    ...
    def flowers_evaluation(self):
        ...
        print('acc: ', accuracy, ' precision: ', [precision_daisy, precision_tulip, precision_rose], ' recall: ',
              [recall_daisy, recall_tulip, recall_rose])

    def sport_evaluation(self):
        error_weight1 = sum(abs(r - c) for r, c in zip(self.__sport['Weight'], self.__sport['PredictedWeight'])) / len(
            self.__sport)
        print('error_weight1: ', error_weight1)
        error_weight2 = sqrt(
            sum((r - c) ** 2 for r, c in zip(self.__sport['Weight'], self.__sport['PredictedWeight'])) / len(
                self.__sport))
        ...
        error_waist2 = sqrt(
            sum((r - c) ** 2 for r, c in zip(self.__sport['Waist'], self.__sport['PredictedWaist'])) / len(
                self.__sport))
