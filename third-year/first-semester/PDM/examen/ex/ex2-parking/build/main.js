...
const spaces = Array.from(Array(30).keys()).map(id => ({
  id,
  number: `s${id}`,
  status: id % 2 === 0 ? 'free' : 'taken',
  takenBy: id % 2 === 0 ? '' : 'u1'
}));
...
router.put('/space/:id', ctx => {
  ...
  if (id !== space.id || index === -1) {
    ...
    ctx.response.body = {
      text: 'Conflict, cannot update status'
    };
...
module.exports = __webpack_require__(/*! D:\Facultate\PDM\examen\ex\ex2-parking\src/index.js */"./src/index.js");
